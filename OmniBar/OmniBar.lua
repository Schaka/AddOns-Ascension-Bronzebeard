...
local GetNumSpecializationsForClassID = function() return 0 end
local GetPlayerInfoByGUID = GetPlayerInfoByGUIDwwwa
local GetRaidRosterInfo = GetRaidRosterInfo
local GetServerTime = GetServerTime
local GetSpecialization = GetSpecialization
local GetSpecializationInfo = GetSpecializationInfo
local GetSpecializationInfoByID = GetSpecializationInfoByID
local GetSpecializationInfoForClassID = function() return nil end
...
local IsRatedBattleground = function() return false end
...
for id, value in pairs(addon.Cooldowns) do
	...
	if not spellName then
		print("OmniBar - incorrect spellId:", id)
	end
	if (spellName and not value.parent) then SPELL_ID_BY_NAME[spellName] = id end
end
