	...
	args = {
		General = {
			...
			args = {
				...
				DisableAlertsForNonHostile = {
					name = L["DisableAlertsForNonHostile"],
					desc = L["DisableAlertsForNonHostileDescription"],
					type = "toggle",
					order = 6,
					width = "full",
					get = function(info)
						return Spy.db.profile.DisableAlertsForNonHostile
					end,
					set = function(info, value)
						Spy.db.profile.DisableAlertsForNonHostile = value
						Spy:ZoneChangedEvent()
					end,
				},
				intro2 = {
...
local Default_Profile = {
	profile = {
		...
		DisableAlertsForNonHostile=false,
...
function Spy:CheckDatabase()
	...
	if Spy.db.profile.DisableAlertsForNonHostile == nil then Spy.db.profile.DisableAlertsForNonHostile = Default_Profile.profile.DisableAlertsForNonHostile end
...
function Spy:OnEnable(first)
	...
	if Spy.db.profile.DisableAlertsForNonHostile then
		Spy:RegisterEvent("NAME_PLATE_UNIT_ADDED", "NamePlateUnitAddedEvent")
		Spy:RegisterEvent("NAME_PLATE_UNIT_REMOVED", "NamePlateUnitRemovedEvent")
	end
...
function Spy:PlayerTargetEvent()
    ...
    if UnitIsPlayer("target") and not SpyPerCharDB.IgnoreData[name] then
        ...
        if UnitIsEnemy("player", "target") then
            if Spy.db.profile.DisableAlertsForNonHostile and not UnitIsPVP("target") then
                return
            end
...
function Spy:PlayerMouseoverEvent()
    ...
    if UnitIsPlayer("mouseover") and not SpyPerCharDB.IgnoreData[name] then
        ...
        if UnitIsEnemy("player", "mouseover") then
            if Spy.db.profile.DisableAlertsForNonHostile and not UnitIsPVP("mouseover") then
                return
            end
            name = strreplace(name, " - ", "-")
...
function Spy:CombatLogEvent(_, timestamp, event, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
	...
	if bit.band(srcFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE
		...
		-- Always check for stealth/prowl regardless of DisableAlertsForNonHostile setting
		if event == "SPELL_AURA_APPLIED" and (select(10, ...) == L["Stealth"] or select(10, ...) == L["Prowl"]) then
			Spy:AlertStealthPlayer(srcName)
		end

		if Spy.db.profile.DisableAlertsForNonHostile and not Spy.HostilePVPList[srcName] then return end
	...
	if bit.band(dstFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE 
		...
		if Spy.db.profile.DisableAlertsForNonHostile and not Spy.HostilePVPList[dstName] then return end
...
function Spy:NamePlateUnitAddedEvent(event, unit)
	if not Spy.EnabledInZone then return end
	if UnitIsPlayer(unit) and UnitIsPVP(unit) and UnitIsEnemy(unit, "player") then
		local name = GetUnitName(unit, true)
		if not name then return end
		Spy.HostilePVPList[name] = true

		if not SpyPerCharDB.PlayerData then SpyPerCharDB.PlayerData = {} end
		if not SpyPerCharDB.IgnoreData then SpyPerCharDB.IgnoreData = {} end

		if not SpyPerCharDB.IgnoreData[name] then
			local playerData = SpyPerCharDB.PlayerData[name]
...
function Spy:NamePlateUnitRemovedEvent(event, unit)
	if UnitIsPlayer(unit) then
		local name = UnitName(unit)
		if Spy.HostilePVPList[name] then
			Spy.HostilePVPList[name] = nil
		end
	end
end
