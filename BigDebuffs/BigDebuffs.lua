...
-- due to different ranks and different Ascension spellIDs, we have to do a hacky workaround and use spell names for matching
...
local CompactUnitFrame_UtilSetDebuff = function(debuffFrame, unit, index, filter, isBossAura, isBossBuff, ...)
    ...
    local name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, _, _, spellId = ...;
    ...
    if index == -1 then
        ...
        if name == nil then
            ...
            if unit then
                if (isBossBuff) then
                    -- name, rank, texture, count, dispelType, duration, expires, caster, isStealable, _, spellID
                    name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, _, _, spellId = UnitBuff(unit,
                        index, filter);
                else
                    name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, _, _, spellId = UnitDebuff(unit
                        , index, filter);
                end
            else
                return;
